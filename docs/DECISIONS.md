...
<a id="d-061"></a>
### D-061 · settled · The viz digest is JSON with base64 payloads, not msgpack

[06-data-model.md](06-data-model.md#viz-digest) specified `digest.msgpack`. The digest is
`digest.json`: scalar series quantized to one or two bytes, base64-encoded, with each block
carrying its own `{bits, min, max, shape}`.

**Why:** msgpack would need a new Python dependency *and* an inlined JavaScript decoder, because
the viewer must be a single self-contained file that works from `file://` with no network access —
a page that cannot fetch a sibling `digest.json` also cannot fetch a decoder. Base64 decodes in a
browser with no library at all. The size argument for msgpack is real (~3× smaller) and does not
bind: the whole digest measures ~1.1 MB against a 5 MB budget, because the quantization does the
compression that the container format would otherwise have to.

**Rejected:** msgpack as written; and carrying both formats, which is two serializers that can
silently disagree about the same run.

**Cost, stated plainly:** if the digest ever grows raster layers for a hundred worlds — the C3 map
view — base64's 33% overhead starts to matter and this should be revisited under a version bump.

*(→ [schemas/digest.md](../schemas/digest.md), [D-013](DECISIONS.md#d-013))*

---

<a id="d-062"></a>
### D-062 · settled · The A2 viewer is one no-build HTML file; TypeScript waits for C3

[11-engineering.md](11-engineering.md) specifies TypeScript + Canvas/WebGL for the Atlas. A2 ships
a single `.html` file with vanilla JS and a 2D canvas, generated by inlining a digest into a
template.

**Why:** A2 is a static picture with no scrubber. A `package.json`, a bundler, and a second
toolchain is a large fixed cost to pay before the thing that needs it — C3's scrubber over
thousands of frames — exists. The property the stack table is actually protecting is *"it must be a
URL you can send someone"*, and one self-contained file has that property more completely than a
built bundle does.

**Rejected:** deferring the viewer entirely and shipping only a PNG. The digest is a published
contract; a contract with no consumer outside its own producer is untested, and the errors surface
at C3 instead of now.

**Revisit at:** C3. The scrubber is where hand-written canvas code stops being cheaper than a
framework.

*(→ [09-visualization.md](09-visualization.md#implementation-notes))*

---

<a id="d-063"></a>
### D-063 · settled · The wall is not sorted by outcome

Strips stack in world order. `--sort survival|final_pop` exists and is never the default.

**Why:** sorting a hundred histories by final population produces a smooth gradient, and a smooth
gradient reads as a finding. It is an artifact of sorting — the same picture appears if the
outcomes are pure noise. The wall's claim is *these diverged*, which unsorted order shows and
sorted order decorates.

This is the visualization-layer form of the rule that produced
[D-054](DECISIONS.md#d-054) and [D-060](DECISIONS.md#d-060): a presentation choice that makes
noise look structured is the same defect as a statistic that does, and it is harder to catch
because nobody computes a z for a sort order.

*(→ [09-visualization.md](09-visualization.md#view-4--the-wall-the-money-shot))*
